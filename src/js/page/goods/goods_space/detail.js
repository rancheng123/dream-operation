...
class GoodsSpaceDetail extends Component {
    ...
    componentDidMount() {
      ...
      switchRoute(){
        const {code} = this.props.routeParams;
            utils.Router.switchRoute('/goods/manage/space/edit/'+code)
    }
    render() {
        ...
        return (
            <div>
                ...
                <div className='flex' style={{width : 172}}><Button type="primary" onClick={this.switchRoute.bind(this)}>编辑</Button><Button style={{backgroundColor : '##E9E9E9'}} onClick={()=>{utils.Router.switchRoute('/goods/manage/space/list/')}}>返回</Button></div>
            </div>
            </div>
        );
    }
