...
class ChecksApplyOrderList extends Component{
    constructor(){
        super();
        this.state = { 
          //'订单审核'
              ...
              render: (text,record) => {
                return (
                  ///goods/manage/space/detail/"+record.step_assignee_id"
                    <Link to={this.getDetailUrlByType(record.issues_id)} style={{textDecoration : 'underline',cursor : 'pointer'}}>{text}</Link>
                )
              }
            },
                ...
                render: (text,record) => {
                    return (
                        <div>
                            ...
                            <Link to={this.getDetailUrlByType(record.issues_id)} style={{cursor : 'pointer'}}><Icon component={Observe} /></Link>
                            </span>
                        </div>
                    )
                }
    ...
    @Bind()
    getDetailUrlByType(issues_id){
          const href = new URL(window.location.href);
          const searchParams = href.searchParams;
          const type_code = searchParams.get('type_code');
          return order_type_code[type_code].path2+issues_id+'?type_code='+type_code
    }
    ...
    getApplyStatusList(){
      api.getApplyStatusList().then(res=>{
        ...
        const {checks_apply_list,dispatch} = this.props;
        checks_apply_list.applyStatusList = res.data;
        dispatch({
          type : 'checks_apply_list',
          data : {
            applyStatusList : checks_apply_list.applyStatusList 
          }
        });
      });
    }
    ...
    getApplyCheckListByCode(){
      ...
      const searchParams = href.searchParams;
      const type_code = searchParams.get('type_code');
      const { pageOption } = checks_apply_list; 
      ...
      //通过审核编码获取审核列表
          ...
          api.getApplyCheckListByCode(pageOption).then((res)=>{
            if(res.code === 10000){
                dispatch({
                    type: 'checks_apply_list',
                    data: {
    ...
    //   let {checks_apply_list,dispatch} = this.props; 
    //   checks_apply_list.pageOption.city_code = locations[0];
    //   checks_apply_list.pageOption.building_code = locations[1];
    //   checks_apply_list.pageOption.palce_code = locations[2];
    //   dispatch({
    //     type : "checks_apply_list",
    //     data : {
    //       pageOption : checks_apply_list.pageOption 
    //     }
    //   });
    // }
    ...
    render(){
        const {checks_apply_list,dispatch} = this.props;
        const { applyStatusList,listData } = checks_apply_list;
        const { data } = listData;  
          ...
          <ShadowBox>
            <SearchConditons
                          data={[
                              {
                                  ...
                                  component: (
                                      <MxjAutoComplete
                                          style={{width: '100%'}}
                                          value={checks_apply_list.autoCompleteOpts.input.value}
                                          selectOpts={{
                                              value:checks_apply_list.autoCompleteOpts.select_order.id,
                                              width: 100,
                                              data: checks_apply_list.autoCompleteOpts.select_order.data,
                                              onChange: (id)=>{

                                                checks_apply_list.autoCompleteOpts.select_order.id = id;
                                                checks_apply_list.pageOption.key_type = id;

                                                  dispatch({
                                                      type: 'checks_apply_list',
                                                      data: {
                                                          autoCompleteOpts: checks_apply_list.autoCompleteOpts,
                                                          pageOption : checks_apply_list.pageOption
                                                      }
                                                  })
                                              }
                                          }}
                                          dataSource={checks_apply_list.autoCompleteOpts.input.dataSource}
                                          onChange={(value) => {

                                            checks_apply_list.autoCompleteOpts.input.value = value;
                                            checks_apply_list.pageOption.key_value = value;
                                              dispatch({
                                                  type: 'checks_apply_list',
                                                  data: {
                                                      autoCompleteOpts: checks_apply_list.autoCompleteOpts,
                                          ...
                                          onSearch={(value) => {
                                              ...
                                              checks_apply_list.autoCompleteOpts.input.dataSource = !value ? [] : [
                                                  value,
                                                  value + value,
                                                  value + value + value,
                                              ];
                                ...
                                component: (
                                    <div style={{width: '100%'}}>
                                      <MxjLocationCascaderForm 
                                        value={checks_apply_list.locationCascader} 
                                        selectType={['locations', 'cities', 'buildings']}
                                        onChange={(values)=>{
                                          checks_apply_list.pageOption.city_code = values.cities;
                                          checks_apply_list.pageOption.build_code = values.buildings;
                                          checks_apply_list.pageOption.location_code = values.locations;
                                          dispatch({
                                            type: 'checks_apply_list',
                                            data: {
                      ...
                      <SearchConditons
                        data={[
                          ...
                          },
                          {
                            ...
                            sm : 12,
                            component: (
                              <Button type="primary" style={{width : 78,float : "right"}} onClick={e=>this.getApplyCheckListByCode()}>查询</Button>
                            ) 
                          }
                        ]}
                      >
              ...
              <MxjTable
                    ...
                    //固定列
                    scroll={{ x: '100%' }}
                    loading={checks_apply_list.loading}
                    pagination={{
                        showQuickJumper: true,
                        showSizeChanger: true,
                        defaultCurrent: 1,
                        pageSize : checks_apply_list.pageOption.per_page,
                        total: checks_apply_list.listData.total,
                        //改变每页条数
                        onShowSizeChange: (current, pageSize)=>{
                          checks_apply_list.pageOption.per_page = pageSize;
                          dispatch({
                              type: 'checks_apply_list',
                              data: {
                                pageOption: checks_apply_list.pageOption
                      ...
                      onChange: (pageNumber)=>{
                        ...
                        checks_apply_list.pageOption.page = pageNumber;
                          dispatch({
                              type: 'checks_apply_list',
                              data: {
                                pageOption: checks_apply_list.pageOption
...
export default connect(function(state) {
  return {
      common: state.common,
      checks_apply_list: state.checks_apply_list
  };
})(ChecksApplyOrderList); 
